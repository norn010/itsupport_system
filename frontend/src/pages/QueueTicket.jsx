          ...
          {inProgressTickets.length > 0 ? inProgressTickets.map(t => (
            <div key={t.id} className="card !p-4 border-l-4 border-indigo-500 bg-white shadow-sm ring-1 ring-slate-100">
              ...
              <div className="flex justify-between items-center text-[10px] font-bold uppercase">
                <div className="text-indigo-600">Assigned: {t.assigned_name || '—'}</div>
                  ...
                  <tr key={t.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="text-[10px] font-black text-slate-400 uppercase leading-tight">
                        ...
                        <div className="text-indigo-600/70">{new Date(t.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            <div key={t.id} className="card !p-4 bg-white/60 backdrop-blur-sm border-slate-200 border shadow-sm">
              <div className="flex justify-between items-center mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold text-slate-400">#{t.ticket_id}</span>
                  {getStatusBadge(t.status)}
                </div>
                <div className="text-[10px] text-slate-400 font-bold">{new Date(t.created_at).toLocaleDateString()}</div>
              </div>
